...
class Authentication {
	...
	public static comparePassword = async (text: string, encryptedText: string): Promise<boolean> => {
	...
	public static generateToken = (id: number, username: string, password: string): string => {
		...
		const token: string = jsonwebtoken.sign({ id, username, password }, secretKey);
